...
// Replace rsr-template-repo, rattlescript, {{DEPS}} with actuals
...
| `$PREFIX/share/rattlescript/`
| Data files, assets

| `$PREFIX/share/doc/rattlescript/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/rattlescript/config.toml`

Fallback: `$HOME/.config/rattlescript/config.toml`
...
Or via OPSM: `opsm update rattlescript`
...
[source,bash]
----
just install --prefix=/opt/rattlescript-instance1 --config=/etc/rattlescript/instance1.toml
just install --prefix=/opt/rattlescript-instance2 --config=/etc/rattlescript/instance2.toml
----
